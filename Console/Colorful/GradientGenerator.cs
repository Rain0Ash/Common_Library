...
namespace Common_Library.Colorful
{
    public sealed class GradientGenerator
    {
        public List<StyleClass<T>> GenerateGradient<T>(IEnumerable<T> input, Color startColor, Color endColor, Int32 maxColorsInGradient)
        {
            ...
            Int32 SetProgressSymmetrically(Int32 remainder)
            {
                return remainder > 1 ? -1 : 0;
            } // An attempt to make the gradient symmetric in the event that maxColorsInGradient does not divide input.Count evenly.

            Int32 ResetProgressSymmetrically(Int32 progress)
            {
            ...
            Boolean IsFirstRun(Int32 index)
            {
                return index == 0;
            }

            Boolean ShouldChangeColor(Int32 index, Int32 progress, T current, T previous)
            {
                return progress > numberOfGrades - 1 && !current.Equals(previous) || IsFirstRun(index);
            }

            Boolean CanChangeColor(Int32 changeCount)
            {
