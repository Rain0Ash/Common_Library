...
namespace Common_Library.LongPath
{
	...
	public class DirectoryInfo : FileSystemInfo
	{
		...
		{
			return Common.IsRunningOnMono() ? SysDirectoryInfo.EnumerateFiles(searchPattern).Select(s => new FileInfo(s.FullName)) : Directory.EnumerateFileSystemEntries(FullPath, searchPattern, false, true, SearchOption.TopDirectoryOnly).Select(e => new FileInfo(e));
		}

		public IEnumerable<FileInfo> EnumerateFiles(String searchPattern, SearchOption searchOption)
		{
			return Common.IsRunningOnMono() ? SysDirectoryInfo.EnumerateFiles(searchPattern, searchOption).Select(s => new FileInfo(s.FullName)) : Directory.EnumerateFileSystemEntries(FullPath, searchPattern, false, true, searchOption).Select(e => new FileInfo(e));
		}
        ...
        {
	        return Common.IsRunningOnMono() ? SysDirectoryInfo.EnumerateDirectories().Select(s => new DirectoryInfo(s.FullName)) : Directory.EnumerateFileSystemEntries(FullPath, "*", true, false, SearchOption.TopDirectoryOnly).Select(directory => new DirectoryInfo(directory));
        }
