...
namespace Common_Library.LongPath
{
    ...
    public static class File
    {
        ...
        public static StreamWriter CreateText(String path)
        {
            ...
            FileStream fileStream = Open(path, FileMode.Create, FileAccess.Write, FileShare.Read, DefaultBufferSize,
        ...
        public static FileStream Create(String path, Int32 bufferSize = DefaultBufferSize)
        {
            return Common.IsRunningOnMono()
                ? SysFile.Create(path, bufferSize)
                : Open(path, FileMode.Create, FileAccess.ReadWrite, FileShare.None, bufferSize, FileOptions.None);
        }

        public static FileStream Create(String path, Int32 bufferSize, FileOptions options)
        {
            return Common.IsRunningOnMono()
                ? SysFile.Create(path, bufferSize, options)
                : Open(path, FileMode.Create, FileAccess.ReadWrite, FileShare.None, bufferSize, options);
        }
        ...
        {
            return Common.IsRunningOnMono()
                ? SysFile.Open(path, mode, access)
                : Open(path, mode, access, FileShare.None, 0, FileOptions.None);
        }
        ...
        {
            return Common.IsRunningOnMono()
                ? SysFile.Open(path, mode, access, share)
                : Open(path, mode, access, share, 0, FileOptions.None);
        }
            ...
            FileOptions options)
        {
            ...
            SafeFileHandle handle = NativeMethods.CreateFile(normalizedPath, underlyingAccess, (UInt32) share, IntPtr.Zero, (UInt32) mode,
                (UInt32) options, IntPtr.Zero);
            if (handle.IsInvalid)
            {
        ...
        /// </remarks>
        internal static StreamReader CreateStreamReader(String path, Encoding encoding, Boolean detectEncodingFromByteOrderMarks,
            Int32 bufferSize)
        {
