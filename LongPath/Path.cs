...
namespace Common_Library.LongPath
{
    public static class Path
    {
        ...
        internal static String NormalizeLongPath(String path, String parameterName)
        {
            ...
            if (path.Length == 0)
            {
                throw new ArgumentException(String.Format(CultureInfo.CurrentCulture, @"'{0}' cannot be an empty string.", parameterName),
                    parameterName);
            }
        ...
        internal static String RemoveLongPathPrefix(String normalizedPath)
        {
            ...
            return normalizedPath.StartsWith(UNCLongPathPrefix, StringComparison.InvariantCultureIgnoreCase)
                ? $@"\\{normalizedPath.Substring(UNCLongPathPrefix.Length)}"
